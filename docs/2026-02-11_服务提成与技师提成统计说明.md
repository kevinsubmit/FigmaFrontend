# 开发更新：服务提成配置 + 技师提成统计（2026-02-11）

## 1. 业务规则
- 每个店铺的每个服务都可单独设置提成金额（`commission_amount`）。
- 提成由店铺管理员在后台店铺服务配置中维护。
- 技师提成统计基于已完成订单（`completed`）和订单拆分（`appointment_staff_splits`）。

## 2. 提成分配算法
当一个订单被拆分给多个技师时：
- 先取该订单对应服务的提成总额：`service.commission_amount`
- 再按技师拆分金额占比进行分配：
  - `技师提成 = 服务提成 * (该技师拆分金额 / 订单拆分总金额)`
